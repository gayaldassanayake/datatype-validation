...
function datetimeInsert(jdbc:Client jdbcClient) returns error?{
    ...
    if(timeCreated is time:Time){
        // time:Time date1 = check time:createTime(2020, 11, 23, 11, 10, 13, 554, "America/Panama");
        
        sql:DateValue date1 = new(timeCreated);
        sql:TimestampValue timestamp1 = new(timeCreated);
        sql:TimestampValue timestamp2 = new(timeCreated);
        sql:TimestampValue timestamp3 = new(timeCreated);
        // sql:DateTimeValue interval_day_to_second = new("4 5:12:10.22");
        // sql:TypedValue dateTyped = <sql:TypedValue> dateAny
        ...
        sql:ParameterizedQuery insertQuery = 
            `INSERT INTO DATETIME(COL_DATE, COL_TIMESTAMP_1, COL_TIMESTAMP_2, COL_TIMESTAMP_3, COL_INTERVAL_YEAR_TO_MONTH, COL_INTERVAL_DAY_TO_SECOND) 
            VALUES (${date1}, ${timestamp1}, ${timestamp2}, ${timestamp3}, ${interval_year_to_month}, ${interval_day_to_second})`;
        sql:ExecutionResult| sql:Error result = check jdbcClient->execute(insertQuery);
    }
}
...
function userdefinedInsert(jdbc:Client jdbcClient) returns sql:Error?{
    ...
    // sql:StructValue struct = new({"attr1": attr1, "attr2":attr2, "attr3": attr3});
...
function xmltypeInsert(jdbc:Client jdbcClient) returns sql:Error?  {
    xml xmlparam = xml `<Warehouse whNo= '100'> <Building>Owned</Building> </Warehouse>`;
