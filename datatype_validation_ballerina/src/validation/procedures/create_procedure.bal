...
function createDateTimeProcedure(jdbc:Client jdbcClient) returns error? {
    string[] variables = ["DATE", "TIMESTAMP", "INTERVAL YEAR TO MONTH", "INTERVAL DAY TO SECOND"];
...
function createANSITypesProcedure(jdbc:Client jdbcClient) returns error? {
    string[] variables = ["CHARACTER", "CHAR", "CHARACTER VARYING", "NATIONAL CHARACTER", 
    "NATIONAL CHAR", "NATIONAL CHARACTER VARYING", "NATIONAL CHAR VARYING", "NCHAR VARYING",
    "NUMERIC", "INTEGER", "INT", "SMALLINT", "FLOAT", "DOUBLE PRECISION", "REAL",
    "DECIMAL"];
...
function createSQLDSProcedure(jdbc:Client jdbcClient) returns error? {
    string[] variables = ["CHARACTER", "VARCHAR", "DECIMAL",
    "INTEGER", "SMALLINT", "FLOAT"];

    string query = createProcedureQuery("SQLDSPROC", variables);
...
function createObjectProcedure(jdbc:Client jdbcClient) returns error? {
    ...
    result = check jdbcClient->execute(
        "CREATE OR REPLACE TYPE OBJECT_TYPE OID '"+ OID2 +"' AS OBJECT(" +
        "ATTR1 VARCHAR(20), "+
        "ATTR2 VARCHAR(20), "+
        "ATTR3 VARCHAR(20), "+
...
function createREFProcedure(jdbc:Client jdbcClient) returns error? {
    ...
    string query = "CREATE OR REPLACE PROCEDURE REFPROC \n"+
        "(VARCHAR_IN REF_TYPE, "+
        "REF_OUT OUT SYS_REFCURSOR ) \n"+
        "AS \n"+
        "BEGIN \n"+
        "OPEN REF_OUT FOR \n"+
        "SELECT VARCHAR_IN FROM SYS.DUAL;\n"+
        "END;";
...
function createVarrayProcedure(jdbc:Client jdbcClient) returns error? {
    sql:ExecutionResult result;
    result = check jdbcClient->execute("DROP TYPE VARRTYPE FORCE");
...
function createXMLProcedure(jdbc:Client jdbcClient) returns error? {
    string[] variables = ["XMLTYPE", "URITYPE", "DBURITYPE", "XDBURITYPE", "HTTPURITYPE"];
...
function createAnyProcedure(jdbc:Client jdbcClient) returns error?{
    string[] variables = ["ANYDATA"];

    string query = createProcedureQuery("ANYPROC", variables);
...
function createAllProcedures(jdbc:Client jdbcClient) returns error? {
    ...
    // err = createANSITypesProcedure(jdbcClient);
    // printCreateProcedureResult("ANSITYPE", err);

    // err = createSQLDSProcedure(jdbcClient);
    // printCreateProcedureResult("SQLDS", err);

    // err = createObjectProcedure(jdbcClient);
    // printCreateProcedureResult("OBJECT", err);

    // err = createREFProcedure(jdbcClient);
    // printCreateProcedureResult("REF", err);

    // err = createVarrayProcedure(jdbcClient);
    // printCreateProcedureResult("VARRAY", err);

    // err = createXMLProcedure(jdbcClient);
    // printCreateProcedureResult("XML", err);

    // err = createAnyProcedure(jdbcClient);
    // printCreateProcedureResult("ANY", err);
}
